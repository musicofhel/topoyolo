...
**Song, Choi & Kahng (2021)**: arXiv: 2109.08918. Machine-learned determination of the transition point and criticality of a hybrid synchronization transition; network structure inferred from chaotic patterns — learned order parameters used as transition detectors rather than analytically derived. Machines: stability (core), matching (weak). Full annotation: `annotations/2109.08918.md` (B2 pass 47). **See also**: `by-domain/statistical_physics.md`.

**Witthaut & Timme (2013)**: arXiv: 1305.1742. Kuramoto dynamics recovered exactly on invariant manifolds of a conservative 2N-variable Hamiltonian system; the synchronization transition is the transverse instability of the action dynamics — the sync boundary recomputed from an embedding instead of dissipative mean-field analysis. Machines: stability (core), joint-vs-marginal excess (transverse dynamics as composite-level excess over the Kuramoto marginal). Full annotation: `annotations/1305.1742.md` (B2 pass 47). **See also**: `by-domain/dynamical_systems.md`.

**Mehta & Schwab (2014)**: arXiv: 1410.3831. Exact variational-RG ↔ RBM mapping on the Ising model: each coarse-graining stage is one layer of the stack; the relevant-operator/irrelevant-operator split at criticality is what "feature learning" performs. Founding claim of the rg-dl lineage, flagged for pairing with the skeptical null 1906.05212 (candidate-10). Machines: parameterized homology (scale as parameter), stability (persistence under coarse-graining). Full annotation: `annotations/1410.3831.md` (B2 pass 49). **See also**: `by-domain/statistical_physics.md`.

**Rançon et al. (2025)**: arXiv: 2506.04016. Generated critical 2D Ising ensembles whose susceptibility, heat capacity and Binder ratios scale correctly and whose RG eigenvalues are nontrivial — criticality reproduced by inversion of the coarse-graining map with three parameters. Machines: stability (persistence certificate), parameterized homology (scale family). Full annotation: `annotations/2506.04016.md` (B2 pass 50). **See also**: `by-domain/statistical_physics.md`.

**de Mello Koch, de Mello Koch & Cheng (2019)**: arXiv: 1906.05212. Correlator diagnostics for RG-like coarse graining in Ising-trained RBMs: RG-like patterns present, but the same observables expose important differences between RG and deep learning — partial refutation of the 1410.3831 founding claim. Machines: null hypothesis (core), stability, parameterized homology. Full annotation: `annotations/1906.05212.md` (B2 pass 51); pair note in `glossary/ANTISYNONYMS.md`. **See also**: `annotations/1410.3831.md`, `by-domain/statistical_physics.md`.

### B2 batch-011: soc — three-way structured disagreement on neural criticality (2026-08-25)

**Plenz, Ribeiro, Miller, Kells, Vakili & Capek (2021)**: arXiv: 2102.09124. CLAIM side: layered cortex self-organizes toward a second-order phase transition — branching parameter ≈ 1 maintained homeostatically (E/I balance, dopamine as control parameters); −3/2 avalanche scaling embedded in up-states and nested oscillations; dissociated layer-less cultures show first-order-like dynamics. Machines: stability (core), parameterized homology. Full annotation: `annotations/2102.09124.md` (B2 pass 57).

**Touboul & Destexhe (2009)**: arXiv: 0910.0805. METHODOLOGICAL NULL side: thresholded stochastic processes generically produce apparent log-log power laws that fail KS scrutiny; surrogates reproduce the scaling; analytic proof of spurious scaling without criticality plus a positive control where true SOC passes rigorous tests. Joins the null-surrogate lineage above (Timmer-class evidence-procedure critique arriving independently in neuroscience). Machines: null hypothesis (core), stability, joint-vs-marginal excess. Full annotation: `annotations/0910.0805.md` (B2 pass 57).

**Dehghani, Hatsopoulos, Haga, Parker, Greger, Halgren, Cash & Destexhe (2012)**: arXiv: 1203.0738. EMPIRICAL NULL side: across cat/monkey/human and wake/sleep, unit avalanches scale exponentially not power-law; nLFP apparent power laws fail CDF-based checks; pLFP control reproduces the artifact; bi-exponential fits win. Negative-stability result: the power-law verdict fails across every measurement channel, species, and brain state tested — while the rejection itself is invariant. Machines: null hypothesis (core), stability, matching (distribution-family selection). Full annotation: `annotations/1203.0738.md` (B2 pass 57).

**Moosavi, Montakhab & Valizadeh (2018)**: arXiv: 1807.07213. MECHANISM/RECONCILIATION side: oscillatory perturbation of the Zhang model yields rhythms embedded in scale-free avalanches; optimal amplification AT the critical point — criticality-as-resonance, coexistence demonstrated inside one known-SOC model (positive control complement to the null lineage). Machines: stability (core), parameterized homology, null hypothesis. Full annotation: `annotations/1807.07213.md` (B2 pass 58).

**Girardi-Schappo, Kinouchi & Tragtenberg (2012)**: arXiv: 1209.3271. MECHANISM + SUBSAMPLING-ARTIFACT side: synaptic noise alone drives critical avalanches in map-based networks (noise-induced rather than self-organized); explicit subsampling analysis — partial recording shapes apparent scaling, the simulation cousin of the empirical measurement-channel critique. Machines: null hypothesis (core), stability, parameterized homology (weak). Full annotation: `annotations/1209.3271.md` (B2 pass 58).

*Role map recorded in `glossary/ANTISYNONYMS.md` ("Power-law avalanche scaling ≠ evidence of self-organized criticality") — the corpus's first structured-disagreement entry. Batch-011 soc group complete: five entries spanning claim / methodological null / empirical null / mechanism-reconciliation / noise-mechanism+subsampling.*
